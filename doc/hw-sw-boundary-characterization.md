...
R0/R1/R2 reductions up to the next irreducible core; exact reductions before
the first RN (or an entire RN-free solve) form one further autonomous epoch,
so C epochs = RN episodes + 1 whenever any reduction precedes the first RN.
`scripts/scaling_characterize.rb` derives its `model_c_epochs` column with the
same rule, so the two reports count Model C identically. Every R0/R1/R2 and
RN event bounds or joins an epoch even when it carries no cost-algebra
elements; R0 in particular is a zero-arithmetic Model B epoch. Model D runs the complete
...
The table shows 20-node corpus means over ten seeds. `epochs/solve` is also the
relevant interaction granularity for B; for C it is the number of
RN/cascade handoffs plus the initial-reduction epoch when one exists; D has
one start interaction. Element percentiles are across epochs, not across
solves: p10/p90 are nearest-rank, the median averages the two middle values
of an even count. Bytes are median/max logical operand plus result traffic
per epoch.

| Family | policy | boundary | epochs / solve | p10 / median / p90 elements | median / max bytes |
|---|---|---|---:|---:|---:|
| degree-3 | min-degree | B atomic reduction | 20.0 | 4 / 8 / 24 | 128 / 264 |
| degree-3 | min-degree | C RN+cascade | 5.0 | 36 / 48 / 48 | 648 / 648 |
| degree-3 | min-degree | D whole heuristic | 1.0 | 228 / 228 / 228 | 3,032 / 3,032 |
| degree-4 | min-degree | B atomic reduction | 20.0 | 4 / 8 / 8 | 128 / 352 |
| degree-4 | min-degree | C RN+cascade | 2.0 | 32 / 94 / 156 | 1,356 / 2,360 |
| degree-4 | min-degree | D whole heuristic | 1.0 | 188 / 188 / 188 | 2,712 / 2,712 |
| mixed-degree | min-degree | B atomic reduction | 20.0 | 12 / 45 / 67 | 492 / 896 |
| mixed-degree | min-degree | C RN+cascade | 15.3 | 33 / 52 / 81 | 552 / 2,004 |
| mixed-degree | min-degree | D whole heuristic | 1.0 | 718 / 853 / 996 | 9,094 / 11,080 |

Degree-3 and degree-4 show the direct benefit of autonomous exact cascades:
Model C reduces handoffs from twenty (nineteen arithmetic reductions plus one
R0) to five or two. Mixed-degree
...
min-degree/min-work; its median C epoch is 48 elements, compared with 94
(the two min-degree epochs are 32 and 156 elements).
