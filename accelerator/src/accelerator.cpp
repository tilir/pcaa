...
bool Accelerator::write_register(uint64_t address, uint32_t value) {
  switch (address) {
    ...
    case ACCEL_MMIO_DESC_ADDR_HI:
      descriptor_address_ =
          (descriptor_address_ & kLow32Mask) |
...
bool Accelerator::execute() {
  ...
  if (descriptor_address_ == 0 ||
      !memory_.read(descriptor_address_, &command, sizeof(command)) ||
  ...
  for (uint32_t i = 0; i < command.n; ++i) {
    ...
    int32_t right = 0;
    int32_t third = 0;
    if (!read_i32(command.src0 + offset, &left) ||
        !read_i32(command.src1 + offset, &right) ||
