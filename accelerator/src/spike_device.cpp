...
class PcaaDevice final : public abstract_device_t {
 public:
  PcaaDevice(simif_t* simulator, reg_t size)
      : memory_(simulator), accelerator_("pcaa_accelerator", memory_), size_(size) {}
 ...
 private:
  bool transport(reg_t address, size_t length, uint8_t* data, tlm::tlm_command command) {
    ...
    accelerator_.b_transport(transaction, delay);
